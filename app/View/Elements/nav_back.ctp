	...
	<ul class="nav navbar-nav back">
		...
		<?php if ($logged_in): ?>
			<li>
				...
				<?php echo $this->Html->link($current_user['user_username'], array('controller' => 'users', 'action' => 'profile', $current_user['user_id']), array('class' => 'dropdown-toggle', 'data-toggle' => 'dropdown', 'aria-expanded' => 'false', "role" => "button", "aria-haspopup"=> "true")
					);
				?>
