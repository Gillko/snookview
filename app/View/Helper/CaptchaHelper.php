 ...
 */
    ...
    function render($settings=array()) {
        ...
        $this->settings['clabel'] = isset( $this->settings['clabel']) ? __($this->settings['clabel']) : __('Enter captcha shown above');

        $this->settings['mlabel'] = isset( $this->settings['mlabel']) ? __($this->settings['mlabel']) : __('<p>Answer Simple Math</p>');

        $this->settings['placeholder'] = isset( $this->settings['placeholder']) ? __($this->settings['placeholder']) : __('Enter captcha shown above');
        ...
        switch( $this->settings['type']):
            case 'image':
                ...
                echo $this->Html->div('unforseen-container');
                    echo $this->Html->image($this->Html->url(array('controller'=>$controller, 'action'=>$action, '?'=> $qstring), true), array('vspace'=>2));
                    echo $this->Html->div('unforseen captcha');
                        echo $this->Html->link($this->settings['reload_txt'], '#', array('class' => 'creload', 'escape' => false));
                    echo '</div>';
                echo '</div>';
                echo $this->Form->input($this->settings['model'].'.'. $this->settings['field'], array('autocomplete'=>'off','label'=> $this->settings['clabel'],'class'=>'clabel form-control','placeholder'=> $this->settings['placeholder'],));
            break;
            case 'math':
