		    	...
		    	<div class="col-md-2 col-sm-6 col-xs-6 player-image noPaddingMobile noPaddingTablet">
		    		<?php if(!empty($player['player_image'])): ?>
			    		<a href="<?php WWW_ROOT ?>/players/<?php echo $player->player_id; ?>/<?php echo $player->player_slug; ?>">
							<?php if($player['player_image']) echo $this->Html->image('/img/players/' . $player['player_image'], array('class' => 'img-responsive', 'alt' => $player['player_firstname'] . ' ' . $player['player_surname'])) . '<p class="img-responsive">' . $player['player_surname']. '</p>'; ?>
						</a>
					<?php else: ?>
						<a href="<?php WWW_ROOT ?>/players/<?php echo $player->player_id; ?>/<?php echo $player->player_slug; ?>">
							<?php echo $this->Html->image('/img/players/Profile.jpg', array('class' => 'img-responsive', 'alt' => $player['player_firstname'] . ' ' . $player['player_surname'])) . '<p class="img-responsive">' . $player['player_surname']. '</p>'; ?>
						</a>
					<?php endif; ?>
		    		<?php //if(!empty($player['player_image'])): {
			   			//echo $this->Html->link(
