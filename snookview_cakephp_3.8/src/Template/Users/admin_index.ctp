		...
		<div class="table-responsive">
			...
			<?php foreach ($users as $user): ?>
				<?php //if($current_user['user_id'] == $user['User']['user_id'] || $current_user['user_role'] == 'admin'): ?>
					<tr>
						<td><?php echo $this->Html->link($user->user_id, ['action' => 'view', $user->user_id]); ?></td>
						<td><?php echo h($user->user_firstname); ?></td>
						<td><?php echo h($user->user_surname); ?></td>
						...
						<td><?php if(!empty($user->user_image)):{
							echo $this->Html->image(h($user->user_image), ['class' => 'thumb']);
						}
						else:{
