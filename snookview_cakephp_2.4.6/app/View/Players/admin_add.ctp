...
<div class="row">
	...
	<?php
		...
		//echo $this->Html->image(h('flags'), array('class' => 'flag'));
		...
		echo $this->Form->end(array('label' => __('Add', true), 'class' => 'btn btn-default btn-success btn-lg'));
	?>
	</fieldset>
	</div>
</div>
...
$('#PlayerPlayerSlug').bind('keypress blur', function() {
	$('#PlayerPlayerRoute').val('Router::connect("/players/' + $('#PlayerPlayerSlug').val() + '", array("controller" => "players", "action" => "view", ' + "<?php echo $lastPlayerID['Player']['player_id']+1; ?>" +'));');
});
</script>